...
# 프로젝트 목적
기존에 존재하는 타 VR 플랫폼인 VR챗에서는 팬미팅을 위한 플랫폼이 아닌 종합 엔터테인먼트 목적 플랫폼이므로 팬미팅 같이 어느정도 관리가 필요한 곳에서는 사용하지 못한다는 단점을 가지고 있다. 
실제로 VR챗에서는 갱이나 테러와 같은 악성 유저를 컨트롤 할수 없다.

따라서 관리자 웹페이지를 통해 유저들을 관리할수 있도록 하는것
